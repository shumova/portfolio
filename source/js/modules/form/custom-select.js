...
const createMultiString = (arr) => {
  // если добавляем не текст, а блоки, то запятая не нужна
  let comma = true;
  if (arr[0] && arr[0].querySelector('[data-select-content]')) {
    comma = false;
  }
...
const setSelectActiveState = (multiple, insert, item) => {
  const buttonTextBlock = item.querySelector('[data-select-text]');
  const activeItems = item.querySelectorAll('[data-select-item][aria-selected="true"]');
  const label = item.querySelector('[data-select-label]');
...
const createSelectStructure = (item) => {
  ...
  const selectItems = item.querySelectorAll('[data-select-item]');
...
const clickAction = (el, index) => {
  const parent = el.closest('[data-select]');
  const multiple = parent.dataset.multiple;
  const insert = parent.dataset.insert;
  const buttonTextBlock = parent.querySelector('[data-select-text]');
  ...
  if (multiple) {
    if (insert === 'true') {
      if (el.getAttribute('aria-selected') === 'true') {
        ...
        const activeItems = parent.querySelectorAll('[data-select-item][aria-selected="true"]');
        const str = createMultiString(activeItems);
    ...
    if (el.getAttribute('aria-selected') === 'true') {
      ...
      buttonTextBlock.innerHTML = itemText;
      el.setAttribute('aria-selected', 'true');
      parent.classList.add('not-empty');
      parent.classList.add('is-valid');
      options[index + 1].selected = true;
...
const setSelectAction = (item) => {
  ...
  const button = customSelect.querySelector('[data-select-button]');
  const selectItems = customSelect.querySelectorAll('[data-select-item]');
